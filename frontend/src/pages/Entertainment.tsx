...
import { api } from '@/api/client'; // ✅ directly use axios client
...
export default function Entertainment() {
  ...
  // ✅ fixed submit handler to prevent reload
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/entertainment/recommend', { title, k: parseInt(k) });
      const data = res.data;
      setResults(data.items || []);
      if (!data.items?.length) toast.info('No recommendations found for this title');
      else toast.success(`Found ${data.items.length} recommendations!`);
    } catch (error: any) {
      console.error(error);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        <FormCard
          ...
          description="Enter a movie title to discover similar films"
        >
          {/* ✅ removed default form reload behavior */}
            ...
            {results.map((movie, idx) => (
              <div key={idx} className="glass hover-lift p-6 rounded-2xl">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {/* ✅ handle both string or array genres */}
                ...
                <div className="pt-3 border-t border-border/50">
                  <p className="text-xs text-muted-foreground">
                    ...
                    <span className="text-primary font-medium">
                      {movie.score ? (movie.score * 100).toFixed(1) : '0'}%
                    </span>
                  </p>
                </div>
              </div>
