...
import { postHealthPredict } from '@/api/client';
...
export default function Health() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await postHealthPredict(payload);
      setResult(data);
                ...
                <div className="text-center p-8 rounded-xl bg-gradient-to-br from-primary/10 to-accent/10 border border-primary/20">
                  ...
                  {result.proba !== undefined && (
                    <p className="text-sm text-muted-foreground">
                      Confidence: {(result.proba * 100).toFixed(1)}%
                    </p>
                  )}
                </div>
              </div>
                  ...
                  <div key={idx} className="p-3 rounded-lg bg-secondary/30 border border-border/50">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-muted-foreground">
                        {item.steps} steps, {item.calories} kcal
                      </span>
                      <span className="text-lg font-bold text-primary">{item.prediction}</span>
                    </div>
                  </div>
                ))}
              </div>
